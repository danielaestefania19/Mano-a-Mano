...
export function useSavingsCircles() {
  ...
  const [allCircles, setAllCircles] = useState<Hash[]>([]);
  const [createdCircles, setCreatedCircles] = useState<Hash[]>([]);
  const [participantCircles, setParticipantCircles] = useState<Hash[]>([]);
  ...
  const fetchCircles = useCallback(async () => {
    ...
    try {
      ...
      setAllCircles(all as Hash[]);
      setTotalCircles(total);

      if (address) {
        const [created, participated] = await Promise.all([
          savingsCircleFactoryService.getCirclesByUser(network, address as Hash),
          savingsCircleFactoryService.getCirclesByParticipant(network, address as Hash),
        ]);

        setCreatedCircles(created as Hash[]);
        setParticipantCircles(participated as Hash[]);
      }
    } catch (err: unknown) {
  ...
  const addCircle = useCallback(
    async (params: CreateCircleParams): Promise<Hash | null> => {
      ...
      try {
        const txHash = await savingsCircleFactoryService.createCircle(network, params);
        await fetchCircles();
        return txHash;
      } catch (err: unknown) {
