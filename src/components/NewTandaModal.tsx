...
const NewTandaModal: React.FC<{ isOpen: boolean; onClose: () => void }> = ({
  ...
  const handleCustomDaysChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!isNaN(days) && days > 0) {
      const durationSec = days * 24 * 60 * 60; // ✅ segundos, no milisegundos
      setFormData((prev) => ({ ...prev, roundDuration: BigInt(durationSec) }));
    }
  };
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.name.trim())
      errors.push("El nombre de la tanda es obligatorio.");
    if (!formData.image) errors.push("Debes subir una imagen para la tanda.");
    if (!amounts.contributionMXN)
      errors.push("El monto de contribución es obligatorio.");
      ...
      <div className="bg-white w-full max-w-md rounded-3xl shadow-2xl p-6 md:p-8 relative overflow-y-auto max-h-[90vh] transition-all">
        ...
        <div className="flex justify-between items-center mb-5 border-b pb-3">
          ...
          <button onClick={onClose}>
            <X
              className="text-gray-500 hover:text-gray-700 hover:bg-gray-200 transition"
              size={22}
            />
          </button>
        </div>
              ...
              <div>
                ...
                {amounts.contributionMXN && ethRate > 0 && (
                  <p className="text-xs text-gray-500 mt-1">
                    ≈ {(Number(amounts.contributionMXN) / ethRate).toFixed(6)}{" "}
                    ETH
                  </p>
                )}
              </div>
