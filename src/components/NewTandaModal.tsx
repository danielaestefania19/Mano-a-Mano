...
const NewTandaModal: React.FC<{ isOpen: boolean; onClose: () => void, refetchParent: () => void }> = ({
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setAmounts({ contributionUSDT: "", insuranceUSDT: "" });
      setPreview(null);
      setValidationErrors([]);
      setFrequency("weekly");
      setCustomDays("");
      ...
      setFormData((prev) => ({ ...prev, roundDuration: BigInt(durationSec) }));
    }
  }, [isOpen]);
  ...
  const handleAmountChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (name === "contributionUSDT") {
      setAmounts({
        contributionUSDT: value,
        insuranceUSDT: value,
      });
      setFormData((prev) => ({
        ...prev,
  ...
  const validateForm = (): boolean => {
    ...
    if (!amounts.contributionUSDT) errors.push("El monto de contribución es obligatorio.");
    ...
    if (Number(amounts.contributionUSDT) < 5)
      errors.push("La contribución mínima debe ser de al menos 5 USDT.");
      ...
      <div className="bg-white w-full max-w-md rounded-3xl shadow-2xl p-6 md:p-8 relative overflow-y-auto max-h-[90vh] transition-all">
        <div className="flex justify-between items-center mb-5 border-b pb-3">
          <div className="flex items-center gap-3">
            <img src={networkLogo} alt={networkName} className="w-7 h-7 rounded-full" />
            <div>
        ...
        <div className="flex justify-between items-center text-sm text-gray-500 mb-4">
          <p>
            1 ETH ≈{" "}
            {ethRate ? (
              <span className="font-semibold text-gray-800">{ethRate.toFixed(2)} USDT</span>
            ) : (
              "Cargando..."
            )}
          </p>
        ...
        {txHash ? (
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
              ...
              {uploadingImage && <p className="text-sm text-gray-500 mt-2">Subiendo imagen a IPFS...</p>}
              {preview && (
                <img
                  src={preview}
                  alt="Preview"
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Contribución (USDT)</label>
                <input
                  type="number"
                  name="contributionUSDT"
                  value={amounts.contributionUSDT}
                  onChange={handleAmountChange}
                  placeholder="100"
                  className="w-full border border-gray-300 rounded-xl p-3 focus:outline-none focus:ring-2 focus:ring-lime-500 transition"
                />
                {amounts.contributionUSDT && ethRate > 0 && (
                  <p className="text-xs text-gray-500 mt-1">
                    ≈ {(Number(amounts.contributionUSDT) / ethRate).toFixed(6)} ETH
                  </p>
                )}
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Depósito de seguro (USDT)</label>
                <input
                  type="number"
                  name="insuranceUSDT"
                  value={amounts.insuranceUSDT}
                  readOnly
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Duración de cada ronda</label>
              <select
                value={frequency}
              ...
              {frequency === "custom" && (
                <div className="mt-3">
                  <input
                    ...
                    className="w-full border border-gray-300 rounded-xl p-3 focus:outline-none focus:ring-2 focus:ring-lime-500 transition"
                  />
                  <p className="text-xs text-gray-500 mt-1">La duración personalizada no puede superar 90 días.</p>
                </div>
              )}
            </div>
